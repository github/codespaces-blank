...
class Solution {
    public int distanceBetweenBusStops(int[] distance, int start, int destination) {
        ...
        // case: start: 7, end: 2
        // we have to swap start and destination to
        // make sure start is always smaller than destination
        if (start > destination) {
            int temp = start;
            start = destination;
            destination = temp;
        ...
        // clockWise distance is result
        // counterClockWise distance is total - reuslt
        int result = 0; 
        int total = 0;
