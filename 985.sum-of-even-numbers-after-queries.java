...
class Solution {
    public int[] sumEvenAfterQueries(int[] A, int[][] queries) {
        ...
        for (int i = 0; i < queries.length; i++) {
            ...
            // if original number is even, we remove it from sum
            // else if it is odd, we dont remove it since we dont even added it
            if (A[index] % 2 == 0) {
                sum -= A[index];
            }
            A[index] += val;
            
            // if new number is even, we add it back to sum
            // else if it is odd, we dont add it back
            if (A[index] % 2 == 0) {
                sum += A[index];
            }
            result[i] = sum;
