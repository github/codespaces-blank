 ...
 * @category Object
 */
export declare function pullObject<T extends IterableContainer, K extends PropertyKey, V>(data: T, keyExtractor: (item: T[number]) => K, valueExtractor: (item: T[number]) => V): Partial<Record<K, V>>;
/**
 * Creates an object that maps the result of `valueExtractor` with a key
 * resulting from running `keyExtractor` on each item in `data`. Duplicate keys
 * are overwritten, guaranteeing that the extractor functions are run on each
 ...
 * @category Object
 */
export declare function pullObject<T extends IterableContainer, K extends PropertyKey, V>(keyExtractor: (item: T[number]) => K, valueExtractor: (item: T[number]) => V): (data: T) => Partial<Record<K, V>>;
//# sourceMappingURL=pullObject.d.ts.map