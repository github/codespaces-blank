 ...
 */
type IsPureTuple<T extends IterableContainer> = T extends readonly [] ? true : T extends readonly [infer Head, ...infer Rest] ? IsUnion<Head> extends true ? false : IsPureTuple<Rest> : false;
/**
 ...
 * }
 */
type IsNarrowable<T, S extends IterableContainer<T>> = IsLiteral<T> extends true ? IsNotFalse<IsPureTuple<S>> : IsNotFalse<IsLiteral<S[number]>>;
/**
 ...
 * @category Guard
 */
export declare function isIncludedIn<T, S extends IterableContainer<T>>(data: T, container: IsNarrowable<T, S> extends true ? S : never): data is S[number];
export declare function isIncludedIn<T, S extends T>(data: T, container: IterableContainer<S>): boolean;
/**
 * Checks if the item is included in the container. This is a wrapper around
 * `Array.prototype.includes` and `Set.prototype.has` and thus relies on the
 * same equality checks that those functions do (which is reference equality,
 ...
 * @category Guard
 */
export declare function isIncludedIn<T, S extends IterableContainer<T>>(container: IsNarrowable<T, S> extends true ? S : never): (data: T) => data is S[number];
export declare function isIncludedIn<T, S extends T>(container: IterableContainer<S>): (data: T) => boolean;
export {};
//# sourceMappingURL=isIncludedIn.d.ts.map