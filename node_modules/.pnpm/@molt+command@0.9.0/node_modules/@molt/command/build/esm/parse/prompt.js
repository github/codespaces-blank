...
export const prompt = (parseProgress, tty) => {
    if (tty === null)
    ...
    for (const spec of specs) {
        ...
        const question = Tex({ flow: `horizontal` })
            ...
            .block((__) => __.block(Term.colors.positive(spec.name.canonical))
        ...
        while (true) {
            ...
            if (validationResult._tag === `Success`) {
                ...
                tty.output(Text.pad(`left`, gutterWidth, ` `, Term.colors.alert(`Invalid value: ${validationResult.errors.join(`, `)}`)));
            }
        }
    }
    // todo do not mutate
