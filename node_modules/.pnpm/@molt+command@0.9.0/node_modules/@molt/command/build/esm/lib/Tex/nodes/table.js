...
export class Table extends Node {
    ...
    render(context) {
        ...
        const maxWidthOfEachColumn = invertTable(rowsAndHeaders).map((col) => Math.max(...col.flatMap(Text.toLines).map((_) => Text.getLength(_))));
        const rowsWithCellWidthsNormalized = rowsAndHeaders.map((row) => {
            ...
            const row__ = row_.map((col, i) => Text.mapLines(col, (line) => Text.padWithin(`right`, maxWidthOfEachColumn[i] ?? 0, ` `, line)));
            return row__;
        });
        const rowsWithCellsJoined = rowsWithCellWidthsNormalized.map((r) => Text.joinColumns(r.map(Text.toLines), separators.column));
