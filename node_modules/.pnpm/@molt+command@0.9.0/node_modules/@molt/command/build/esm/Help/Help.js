...
export const render = (specs_, settings, _settings) => {
    ...
    const mexGroups = Object.values(groupBy(specsByKind.Exclusive ?? [], (_) => _.group.label)).map((_) => _[0].group);
    const noteItems = [];
    ...
    const output = Tex.Tex({ maxWidth: 82, padding: { bottom: 0, top: 0 } })
        ...
        .block({ padding: { left: 2 } }, (__) => __.table({ separators: { column: `   `, row: null } }, (__) => __.header({ padding: { right: 2, bottom: 1 } }, chalk.underline(Term.colors.mute(columnTitles.name)))
        .header({
        minWidth: 8,
        padding: { right: 5 },
        ...
        .header(columnTitles.environment ? chalk.underline(Term.colors.mute(columnTitles.environment)) : null)
        .rows([
        ...basicAndUnionSpecsWithoutHelp.map((spec) => [
            parameterName(spec),
            Tex.block({ maxWidth: 40, padding: { right: 9, bottom: 1 } }, parameterTypeAndDescription(settings, spec)),
        ...
        ...mexGroups.flatMap((mexGroup) => {
            ...
            return [
                [
                    Tex.block({ border: { left: Term.colors.dim(`┌`) } }, Term.colors.dim(`─${mexGroup.label} ${`(2)`}`)),
                    ``,
                    default_,
                ],
...
const environmentNote = (specs, settings) => {
    const isHasSpecsWithCustomEnvironmentNamespace = specs
        ...
        .filter((_) => _.environment.namespaces.filter((_) => settings.parameters.environment.$default.prefix.map(camelCase).includes(_)).length !== _.environment.namespaces.length).length > 0;
    let content = ``;
    content +=
    ...
    if (settings.parameters.environment.$default.prefix.length > 0) {
        if (isHasSpecsWithCustomEnvironmentNamespace) {
            ...
            content += ` ${Text.joinListEnglish(settings.parameters.environment.$default.prefix.map((_) => Term.colors.secondary(Text.toEnvarNameCase(_) + `_`)))} (case insensitive), though some parameters deviate (shown in docs). `;
        }
        else {
            content += `They must be prefixed with`;
            content += ` ${Text.joinListEnglish(settings.parameters.environment.$default.prefix.map((_) => Term.colors.secondary(Text.toEnvarNameCase(_) + `_`)))} (case insensitive). `;
        }
    }
    else {
    ...
    const examples = specs
        ...
        ? `${Term.colors.secondary(Text.toEnvarNameCase(_.environment.namespaces[0]) + `_`)}${Term.colors.positive(Text.toEnvarNameCase(_.name.canonical))}`
        : Term.colors.positive(Text.toEnvarNameCase(_.name.canonical)))
        .map((_) => `${_}="..."`);
    return Tex.block(($) => $.text(content).list({
        padding: { left: 2 },
        bullet: {
...
const parameterName = (spec) => {
    ...
    return Tex.block(parameters, (__) => __.block(isRequired ? Term.colors.positiveBold(spec.name.canonical) : Term.colors.positive(spec.name.canonical))
...
const parameterTypeAndDescription = (settings, spec) => {
    if (spec._tag === `Union`) {
        ...
        if (isExpandedMode) {
            ...
            return Tex.block((__) => __.block(Term.colors.dim(Text.chars.borders.leftTop + Text.chars.borders.horizontal + `union`))
                .block({ padding: { bottom: 1 }, border: { left: `${Term.colors.dim(Text.chars.borders.vertical)} ` } }, spec.description)
                .block(types)
    ...
    return Tex.block({ padding: { bottom: spec._tag === `Exclusive` ? 0 : 1 } }, ($) => $.block(typeScalar(spec.type)).block(spec.description));
};
        ...
        ? Term.colors.secondary(Text.chars.check) +
            (spec.environment.enabled && spec.environment.namespaces.length === 0
                ...
                : spec.environment.enabled &&
                    ...
                    (_) => !settings.parameters.environment.$default.prefix.includes(snakeCase(_))).length > 0
                    ? ` ` +
                        Term.colors.dim(spec.environment.namespaces
