...
exports.parse = exports.lookupEnvironmentVariableArgument = exports.defaultParameterNamePrefixes = void 0;
...
const parse = (environment, specs) => {
    const envars = Object.entries(environment)
        ...
        ? value
        : {
            name: { raw: name, camel: (0, lodash_camelcase_1.default)(name) },
            value,
        })
    ...
    for (const envar of envars) {
        for (const spec of specsPool) {
            ...
            if (isNamespaced) {
                const specParameterNames = index_js_1.ParameterSpec.getNames(spec);
                // eslint-disable-next-line
                const namespaceAndNameHit = spec
                    ...
                    (0, lodash_camelcase_1.default)(`${namespace}_no_${name}`),
                    (0, lodash_camelcase_1.default)(`${namespace}_${name}`),
                ])
                ...
                if (namespaceAndNameHit) {
                    ...
                    const value = (0, helpers_js_1.parseRawInput)(envar.name.camel, envar.value, spec);
                    // dump(typeof report)
                    if (report) {
