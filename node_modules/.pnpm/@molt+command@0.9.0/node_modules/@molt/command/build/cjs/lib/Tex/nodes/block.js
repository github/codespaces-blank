...
class Block extends node_js_1.Node {
    constructor(...args) {
        super();
        ...
        if (typeof children === `string`) {
            this.children = [new leaf_js_1.Leaf(children)];
        }
    ...
    render(context) {
        ...
        if (context.phase === `inner` || !context.phase) {
            const widthOwn = typeof this.parameters.width === `number`
                ? { type: `absolute`, value: this.parameters.width }
            ...
            const maxWidthResolved = Math.min(widthOwnResolved ?? Infinity, maxWidthOwn, context.maxWidth ?? 1000) - paddingLeftOwn;
            ...
            for (const child of this.children) {
                ...
                if (this.parameters.minWidth !== undefined) {
                    rendered.value = index_js_1.Text.mapLines(rendered.value, (_) => index_js_1.Text.minSpan(`left`, this.parameters.minWidth, _));
                }
            ...
            let width = 0;
            if (flow === `horizontal`) {
                joined = index_js_1.Text.row(renderings.map((_) => ({
                    lines: _.split(`\n`),
                    separator: ``,
                })));
                width = index_js_1.Text.measure(joined).maxWidth;
            }
            else {
                width = widthOwnResolved === null ? intrinsicWidth : maxWidthResolved;
                // each line must span the width of the box
                renderings = renderings.map((_) => index_js_1.Text.minSpan(`left`, width, _));
                joined = renderings.join(index_js_1.Text.chars.newline);
            }
            let value = joined;
            if (this.parameters.padding) {
                value = (0, helpers_js_1.applyPadding)(value, this.parameters.padding, context);
            }
            const intrinsicHeight = index_js_1.Text.toLines(value).length;
        ...
        if (context.phase === `outer` || !context.phase) {
            ...
            const lines = index_js_1.Text.toLines(value);
            ...
            const selfMeasure = hadVerticalBorders
                ? index_js_1.Text.measure(index_js_1.Text.fromLines(linesWithBorders))
            ...
            const width = selfMeasure.maxWidth - index_js_1.Text.getLength(corners) * 2;
            ...
            const borderBottom = this.parameters.border?.bottom
                ? corners +
                    ...
                    corners
                : null;
            const linesRendered = [borderTop, linesWithBorders.join(index_js_1.Text.chars.newline), borderBottom]
                .filter(Boolean)
            ...
            //     )
            //   : value
            const color = this.parameters.color ?? ((text) => text);
            value = color(value);
            const { maxWidth: intrinsicWidth, height: intrinsicHeight } = index_js_1.Text.measure(value);
