...
export const runPipeline = async ({ core, hookNamesOrderedBySequence, originalInput, extensionsStack, asyncErrorDeferred }) => {
    ...
    const done = createDeferred({ strict: false });
    void runHook({
        core,
        name: hookName,
        done: done.resolve,
    ...
    const signal = await Promise.race([done.promise, asyncErrorDeferred.promise]);
    switch (signal.type) {
        case `completed`: {
...
const runPipelineEnd = async ({ extensionsStack, result, }) => {
