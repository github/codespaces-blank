...
export const { moduleName: moduleNameGlobal, generate: generateGlobal } = createCodeGenerator(`Global`, (config) => {
    ...
    if (config.typeMapByKind.GraphQLScalarTypeCustom.length > 0) {
        code.push(`import type * as CustomScalar from '${config.importPaths.customScalarCodecs}'`);
    }
    code.push(`
    declare global {
      export namespace GraphQLRequestTypes {
        ...
        .map((_) => {
        return `${_.name}: ${needsDefaultCustomScalarImplementation ? `${StandardScalarNamespace}.String` : `CustomScalar.${_.name}`}`;
    }).join(`\n`)}
          }
          featureOptions: {
