...
export type OmitWithTag2<TaggedRecord extends SomeTaggedRecord> = Omit<TaggedRecord, GetTagProperty<TaggedRecord>>;
...
export type DecoderDefinition<V extends SomeStoredRecord> = (encodedData: string, extensions: V[`extensions`] & {
    schema: V['schema'];
    name: V[`name`];
...
export declare namespace StoredRecords {
    /**
     * Get the methods for decoders that are defined across all records.
     */
    export type GetAdtLevelDecoderMethods<Vs extends StoredRecords> = UnionToIntersection<ObjectValues<{
     ...
     */
    export type GetAdtLevelEncoderMethods<Rs extends StoredRecords> = UnionToIntersection<ObjectValues<{
